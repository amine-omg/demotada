...
<template>
  <header class="h-16 md:h-24 bg-[#050505] px-4 md:px-10 flex items-center justify-between sticky top-0 z-50 w-full shrink-0">
    ...
    <div class="flex items-center gap-3 md:gap-5 min-w-0">
      ...
      <h1 class="text-base md:text-xl font-black text-white tracking-tighter italic truncate">
        {{ pageTitle }}
      </h1>
    </div>
    ...
    <div class="flex items-center gap-2 md:gap-6 shrink-0">
      ...
      <div class="relative notification-dropdown">
        <button 
          ...
          class="relative p-2 md:p-3 text-slate-500 hover:text-[#E5E5D1] hover:bg-white/5 rounded-xl md:rounded-2xl transition-all duration-300"
        >
        ...
        <transition name="menu-fade">
          <div v-if="showNotificationsMenu" class="absolute right-0 mt-3 md:mt-4 w-[90vw] sm:w-[350px] md:w-[400px] max-w-[400px] bg-[#0A0A0A] border border-white/10 rounded-[1.5rem] md:rounded-[2rem] shadow-[0_20px_40px_rgba(0,0,0,0.9)] overflow-hidden z-50 origin-top-right">
            <div class="px-5 py-4 md:px-7 md:py-5 border-b border-white/5 flex justify-between items-center bg-[#E5E5D1]/5">
              <span class="text-sm md:text-base font-black text-white uppercase tracking-widest">Flux</span>
              <span v-if="notificationStore.unreadCount > 0" class="text-[9px] md:text-[10px] bg-[#E5E5D1] text-[#050505] px-2 md:px-2.5 py-1 rounded-full font-black uppercase tracking-tighter">
                {{ notificationStore.unreadCount }} nouveaux
              </span>
            </div>

            <div class="max-h-[60vh] md:max-h-[450px] overflow-y-auto custom-scrollbar">
              <div v-if="isLoadingNotifications" class="p-8 md:p-10 text-center text-slate-500 text-xs md:text-sm italic">Analyse en cours...</div>
              <div v-else-if="notifications.length === 0" class="p-8 md:p-10 text-center text-slate-500 text-xs md:text-sm italic">Aucun mouvement.</div>
              ...
              <div v-else>
                <div
                  ...
                  class="flex items-start gap-3 md:gap-4 px-4 py-4 md:px-7 md:py-5 border-b border-white/[0.03] hover:bg-white/[0.05] cursor-pointer transition-colors relative group"
                ...
                >
                  <div class="flex-shrink-0 mt-1">
                    ...
                    <div v-else class="w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl bg-[#E5E5D1]/10 flex items-center justify-center text-[#E5E5D1]">
                      <span class="text-sm md:text-lg">🐑</span>
                    </div>
                  </div>
                  
                  <div class="flex-1 min-w-0">
                    <p class="text-xs md:text-base leading-snug" :class="{ 'text-white font-bold': !notification.read, 'text-slate-400': notification.read }">
                      {{ notification.message }}
                    </p>
                    <p class="text-[9px] md:text-[11px] text-slate-500 mt-1 md:mt-2 font-bold uppercase tracking-wider opacity-60">{{ new Date(notification.createdAt).toLocaleDateString() }}</p>
                  </div>

                  <div class="flex flex-col gap-2 md:gap-3 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button @click="notificationStore.markAllAsRead()" class="w-full py-4 md:py-5 text-[9px] md:text-[11px] font-black text-slate-500 hover:text-[#E5E5D1] hover:bg-[#E5E5D1]/5 transition-all uppercase tracking-[0.3em]">
      ...
      <div class="w-[1px] h-6 md:h-8 bg-white/10 mx-1 md:mx-2"></div>

      <div class="relative profile-dropdown">
        <button 
          @click.stop="toggleProfile"
          class="flex items-center gap-2 md:gap-4 p-1 md:pl-2 md:pr-5 md:py-2 rounded-xl md:rounded-2xl bg-transparent md:bg-white/5 border border-transparent md:border-white/10 md:hover:border-[#E5E5D1]/40 transition-all duration-300 group"
        >
          <div class="h-8 w-8 md:h-11 md:w-11 rounded-lg md:rounded-xl ring-2 ring-[#E5E5D1]/10 ring-offset-2 md:ring-offset-4 ring-offset-[#050505] overflow-hidden bg-[#E5E5D1] flex items-center justify-center shadow-[0_0_15px_rgba(229,229,209,0.1)]">
          ...
          <svg class="hidden md:block w-4 h-4 text-slate-600 group-hover:text-[#E5E5D1] transition-transform duration-500" :class="{ 'rotate-180': showProfileMenu }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
          </svg>
        </button>

        <transition name="menu-fade">
          <div v-if="showProfileMenu" class="absolute right-0 mt-3 md:mt-4 w-[85vw] max-w-[280px] md:w-72 md:max-w-none bg-[#0A0A0A] border border-white/10 rounded-[1.5rem] md:rounded-[2rem] shadow-[0_20px_40px_rgba(0,0,0,0.9)] overflow-hidden z-50 origin-top-right">
            <div class="p-5 md:p-6 border-b border-white/5 bg-gradient-to-b from-[#E5E5D1]/5 to-transparent">
            ...
            <div class="p-2 md:p-3">
              <router-link to="/mon-compte" class="flex items-center gap-3 md:gap-4 px-3 py-3 md:px-4 md:py-4 text-sm md:text-base text-slate-400 hover:bg-[#E5E5D1]/10 hover:text-white rounded-xl md:rounded-2xl transition-all group">
                <div class="p-2 md:p-2.5 rounded-lg md:rounded-xl bg-white/5 group-hover:bg-[#E5E5D1]/20 text-[#E5E5D1]">
              ...
              <button @click="handleLogout" class="w-full mt-1 md:mt-2 flex items-center gap-3 md:gap-4 px-3 py-3 md:px-4 md:py-4 text-sm md:text-base text-red-900/80 hover:bg-red-500/10 hover:text-red-500 rounded-xl md:rounded-2xl transition-all group">
                <div class="p-2 md:p-2.5 rounded-lg md:rounded-xl bg-red-500/5 group-hover:bg-red-500/20">
