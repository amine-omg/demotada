        ...
        <div v-else class="flex flex-col gap-4 min-w-0">
          ...
          >
            ...
            <div class="mt-3 flex items-center gap-3 text-[8px] font-black uppercase tracking-widest text-gray-400 min-w-0 pl-1">
              <span class="flex items-center gap-1 truncate"><svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>{{ ent.siret }}</span>
              <span class="flex items-center gap-1 truncate" v-if="ent.ville"><svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/></svg>{{ ent.ville }}</span>
            </div>
          </button>
        </div>
      </div>
          ...
          <div class="mb-6 min-w-0 text-left">
             <div class="flex items-center gap-2 mb-3">
                <span class="px-2.5 py-1 bg-orange-100 text-orange-700 border border-orange-200 rounded-lg text-[8px] font-black uppercase tracking-widest flex items-center gap-1 shrink-0">
                  <span class="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse shrink-0"></span>EN ATTENTE
                </span>
    ...
    <div v-else class="flex-1 max-w-[1400px] mx-auto w-full p-8 mt-4 grid grid-cols-12 gap-8 h-[calc(100vh-6rem)]">
            ...
            <div class="flex justify-between items-start pl-2">
              <div class="flex items-center gap-2 flex-wrap pr-4">
                ...
                <span v-if="ent.qualifications?.rgeValide" class="bg-green-50 text-green-700 border border-green-200 px-1.5 py-0.5 rounded text-[7px] font-black uppercase tracking-widest flex items-center gap-1 shadow-sm shrink-0">
                  <svg class="w-2 h-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path d="M5 13l4 4L19 7"/></svg>
            ...
            <div class="pl-2 mt-2">
              ...
              <div class="mt-2 flex items-center gap-4 text-[9px] font-black uppercase tracking-widest text-gray-400">
                <span class="flex items-center gap-1">
                  <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                  {{ ent.siret }}
                </span>
      ...
      <div class="col-span-7 bg-white border-2 border-black rounded-2xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col h-full relative overflow-hidden z-0 min-w-0">
        ...
        <div v-if="!selectedEnt" class="flex-1 flex flex-col items-center justify-center text-gray-300 bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAiIGhlaWdodD0iMjAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGNpcmNsZSBjeD0iMSIgY3k9IjEiIHI9IjEiIGZpbGw9InJnYmEoMCwwLDAsMC4wNSkiLz48L3N2Zz4=')]">
          <div class="w-20 h-20 bg-white border-2 border-dashed border-gray-300 rounded-full flex items-center justify-center mb-4 shrink-0">
             <svg class="w-8 h-8 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
          </div>
        ...
        <template v-else>
          <div class="p-8 border-b-2 border-black bg-white shrink-0 relative z-10 text-left min-w-0">
            <div class="absolute top-8 right-8 text-right flex flex-col items-end gap-2 shrink-0">
            ...
            <div class="flex items-center gap-3 mb-3 shrink-0">
              <span class="px-2.5 py-1 bg-orange-100 text-orange-700 border border-orange-200 rounded text-[8px] font-black uppercase tracking-widest flex items-center gap-1 shadow-sm shrink-0">
                <span class="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse shrink-0"></span>En attente
              </span>
              <span class="text-[10px] font-bold text-gray-400">Inscrit le {{ new Date(selectedEnt.createdAt).toLocaleDateString('fr-FR') }}</span>
            </div>
            <h1 class="text-3xl font-black uppercase tracking-tighter text-[#1A1A1A] max-w-[80%] leading-tight break-words">{{ selectedEnt.raisonSociale }}</h1>
            <p class="text-xs font-bold text-gray-500 mt-2 flex items-center gap-2 truncate">
              <svg class="w-4 h-4 text-[#D4AF37]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/></svg>
          ...
          <div class="px-8 py-4 bg-gray-50 border-b-2 border-black shrink-0 flex gap-4 z-10 shadow-inner">
            <button 
              @click="traiterDemande('refuse')" 
              :disabled="isProcessing"
              class="flex-1 bg-white text-red-600 border-2 border-red-200 hover:border-red-600 hover:bg-red-50 font-black uppercase tracking-widest text-[10px] py-4 rounded-xl transition-all shadow-sm hover:shadow-[4px_4px_0px_0px_rgba(220,38,38,1)] flex items-center justify-center gap-2 disabled:opacity-50"
            >
            ...
            <button 
              @click="traiterDemande('valide')" 
              :disabled="isProcessing"
              class="flex-[2] bg-[#1A1A1A] hover:bg-black text-green-400 font-black uppercase tracking-widest text-[10px] py-4 rounded-xl transition-all shadow-[4px_4px_0px_0px_rgba(74,222,128,0.4)] hover:shadow-[4px_4px_0px_0px_rgba(74,222,128,1)] flex items-center justify-center gap-2 hover:-translate-y-0.5 disabled:opacity-50"
            >
              <span v-if="isProcessing" class="w-5 h-5 border-2 border-green-400/30 border-t-green-400 rounded-full animate-spin"></span>
              <template v-else>
          ...
          <div class="flex-1 overflow-y-auto p-8 bg-[#F8F9FA] relative text-left min-w-0">
            <div v-if="selectedEnt.estActive === false" class="mb-8 p-5 bg-red-50 border-2 border-red-200 rounded-2xl shadow-[6px_6px_0px_0px_rgba(220,38,38,0.2)] flex items-start gap-4 animate-fade-in">
              <div class="w-12 h-12 bg-red-100 text-red-600 rounded-full flex items-center justify-center shrink-0"><svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg></div>
              <div class="min-w-0"><h3 class="text-sm font-black text-red-700 uppercase tracking-widest">Alerte : Société Radiée</h3><p class="text-xs font-bold text-red-600 mt-1 leading-tight">Ce SIRET n'est plus valide pour déposer des CEE.</p></div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-8 min-w-0">
              <div class="space-y-8">
                <div class="bg-white border-2 border-black rounded-2xl p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                  <h2 class="text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 pb-2 mb-5 flex items-center gap-2"><svg class="w-4 h-4 text-[#D4AF37]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2"/></svg>Identité SIRENE</h2>
                  <div class="space-y-4">
                    <div><label class="block text-[9px] font-black uppercase text-gray-400 mb-1">SIRET</label><div class="w-full bg-gray-50 border border-gray-200 rounded-lg p-2.5 text-sm font-black font-mono text-[#1A1A1A] truncate">{{ selectedEnt.siret }}</div></div>
                    <div><label class="block text-[9px] font-black uppercase text-gray-400 mb-1">NAF & Activité</label><div class="w-full bg-gray-50 border border-gray-200 rounded-lg p-2.5 text-xs font-bold text-[#1A1A1A] break-words">{{ selectedEnt.codeNaf }} - {{ selectedEnt.libelleNaf || 'N/C' }}</div></div>
                  </div>
                </div>

                <div v-if="selectedEnt.type !== 'client_tertiaire'" class="bg-white border-2 border-black rounded-2xl p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                  <h2 class="text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 pb-2 mb-5 flex items-center gap-2"><svg class="w-4 h-4 text-[#D4AF37]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>Audit & Qualité</h2>
                  <div class="flex items-center gap-5">
                    <div class="w-16 h-16 rounded-full border-[3px] flex items-center justify-center bg-gray-50 shrink-0" :class="selectedEnt.auditQualite?.noteMoyenne >= 8 ? 'border-green-400 text-green-600' : 'border-gray-200 text-gray-400'"><span class="text-xl font-black">{{ selectedEnt.auditQualite?.noteMoyenne || '-' }}</span></div>
                    <div class="min-w-0"><p class="font-black uppercase text-sm text-[#1A1A1A] truncate">{{ selectedEnt.auditQualite?.statut || 'Nouveau' }}</p><p class="text-[10px] text-gray-500 font-bold mt-1 uppercase tracking-wider truncate">Aucune fraude détectée.</p></div>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-8">
                <div class="bg-white border-2 border-black rounded-2xl p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]" v-if="selectedEnt.contacts && selectedEnt.contacts.length > 0">
                  <h2 class="text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 pb-2 mb-5 flex items-center gap-2 truncate"><svg class="w-4 h-4 text-[#D4AF37] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>Contact</h2>
                  <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-3"><div class="min-w-0"><label class="block text-[8px] font-black uppercase text-gray-400 mb-1 truncate">Prénom Nom</label><div class="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 text-xs font-bold text-[#1A1A1A] truncate">{{ selectedEnt.contacts[0].prenom }} {{ selectedEnt.contacts[0].nom }}</div></div><div class="min-w-0"><label class="block text-[8px] font-black uppercase text-gray-400 mb-1 truncate">Fonction</label><div class="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 text-xs font-bold text-[#1A1A1A] truncate">{{ selectedEnt.contacts[0].fonction || '---' }}</div></div></div>
                    <div class="min-w-0"><label class="block text-[8px] font-black uppercase text-gray-400 mb-1 truncate">Email</label><div class="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 text-xs font-bold text-[#1A1A1A] truncate">{{ selectedEnt.contacts[0].email || 'N/C' }}</div></div>
                  </div>
                </div>

                <div v-if="selectedEnt.qualifications?.rgeValide" class="bg-white border-2 border-black rounded-2xl p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                  <h2 class="text-[10px] font-black uppercase tracking-widest text-gray-400 border-b border-gray-100 pb-2 mb-4 flex items-center gap-2"><svg class="w-4 h-4 text-[#D4AF37]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Label RGE</h2>
                  <div class="p-5 bg-green-50 border-2 border-green-500 rounded-xl relative overflow-hidden">
                    <div class="flex items-start gap-4 z-10"><div class="w-10 h-10 bg-green-500 text-white rounded-full flex items-center justify-center shrink-0 shadow-md"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path d="M5 13l4 4L19 7"/></svg></div><div><h3 class="text-sm font-black text-green-800 uppercase tracking-widest truncate">RGE Validé</h3><p class="text-[10px] text-green-700 font-bold mt-1 truncate">Certifications actives via ADEME.</p></div></div>
                  </div>
                </div>
              </div>
            </div>
